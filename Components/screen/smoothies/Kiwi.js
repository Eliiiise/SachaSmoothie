...
export default function Kiwi({route , navigation}) {
    ...
    const displayData = async ()=>{
        try{
            ...
            let nbMy = await AsyncStorage.getItem('Kiwi');
            nbMy = parseInt(nbMy)+1;
            setMy(nbMy);
            AsyncStorage.setItem('Kiwi', JSON.stringify(nbMy) );
        }
        catch(error){
            alert(nbPanier)
        }
    ...
    return (
        <KeyboardAvoidingView style={styles.container} behavior="position" enabled>
            ...
            <Image  style={{width: 180, height: 300, marginTop:-60, alignSelf:'center'}} source={require("../../../assets/bleu.png")} />
            <Text style={[styles.title, { marginTop:-30}]}>
                Le smoothie Kiwi Bleu est fait  {"\n"}pour toi !
            </Text>
            <Text style={styles.text}>
                Tu vas faire des jaloux {name} !{"\n"} Ce smoothie est composé de Kiwi et de mûre.
            </Text>
