...
export default function Avocat({route , navigation}) {
    ...
    const displayData = async ()=>{
        try{
            ...
            let nbMy = await AsyncStorage.getItem('Avocat');
            nbMy = parseInt(nbMy)+1;
            setMy(nbMy);
            AsyncStorage.setItem('Avocat', JSON.stringify(nbMy) );
        }
        catch(error){
            alert(nbPanier)
        }
    ...
    return (
        <KeyboardAvoidingView style={styles.container} behavior="position" enabled>
            ...
            <Image  style={{width: 180, height: 300, marginTop:-60, alignSelf:'center'}} source={require("../../../assets/vert.png")} />
            <Text style={[styles.title, { marginTop:-30}]}>
                Le smoothie Force Verte est fait  {"\n"}pour toi !
            </Text>
            <Text style={styles.text}>
                Tu vas faire des jaloux {name} !{"\n"} Ce smoothie est composé d'avocat de concombre et de romarin.
            </Text>
