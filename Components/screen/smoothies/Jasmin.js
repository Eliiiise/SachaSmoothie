...
export default function Jasmin({route , navigation}) {
    ...
    const displayData = async ()=>{
        try{
            ...
            let nbMy = await AsyncStorage.getItem('Jasmin');
            nbMy = parseInt(nbMy)+1;
            setMy(nbMy);
            AsyncStorage.setItem('Jasmin', JSON.stringify(nbMy) );
        }
        catch(error){
            alert(nbPanier)
        }
    ...
    return (
        <KeyboardAvoidingView style={styles.container} behavior="position" enabled>
            ...
            <Image  style={{width: 180, height: 300, marginTop:-60, alignSelf:'center'}} source={require("../../../assets/marron.png")} />
            <Text style={[styles.title, { marginTop:-30}]}>
                Le smoothie des Tropiques est fait{"\n"}pour toi !
            </Text>
            <Text style={styles.text}>
                Tu vas faire des jaloux {name} !{"\n"} Ce smoothie est composé de thé au jasmin et d'ananas.
            </Text>
