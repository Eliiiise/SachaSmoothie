...
export default function Poivron({route , navigation}) {
    ...
    const displayData = async ()=>{
        try{
            ...
            let nbMy = await AsyncStorage.getItem('Poivron');
            nbMy = parseInt(nbMy)+1;
            setMy(nbMy);
            AsyncStorage.setItem('Poivron', JSON.stringify(nbMy) );
        }
        catch(error){
            alert(nbPanier)
        }
    ...
    return (
        <KeyboardAvoidingView style={styles.container} behavior="position" enabled>
            ...
            <Image  style={{width: 180, height: 300, marginTop:-60, alignSelf:'center'}} source={require("../../../assets/orange.png")} />
            <Text style={[styles.title, { marginTop:-30}]}>
                Le smoothie Pimenté est fait  {"\n"}pour toi!
            </Text>
            <Text style={styles.text}>
                Tu vas faire des jaloux {name} !{"\n"} Ce smoothie est composé de poivron, de mangue et de carotte.
            </Text>
