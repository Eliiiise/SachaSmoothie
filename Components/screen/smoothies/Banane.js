...
export default function Banane({route , navigation}) {
    ...
    const displayData = async ()=>{
        try{
            ...
            let nbMy = await AsyncStorage.getItem('Banane');
            nbMy = parseInt(nbMy)+1;
            setMy(nbMy);
            AsyncStorage.setItem('Banane', JSON.stringify(nbMy) );
        }
        catch(error){
            alert(nbPanier)
        }
    ...
    return (
        <KeyboardAvoidingView style={styles.container} behavior="position" enabled>
            ...
            <Image  style={{width: 180, height: 300, marginTop:-60, alignSelf:'center'}} source={require("../../../assets/jaune.png")} />
            <Text style={[styles.title, { marginTop:-30}]}>
                Le smoothie Gourmand est fait{"\n"}pour toi !
            </Text>
            <Text style={styles.text}>
                Tu vas faire des jaloux {name} !{"\n"} Ce smoothie est composé de banane, beurre de cacahuète et d'amande.
            </Text>
